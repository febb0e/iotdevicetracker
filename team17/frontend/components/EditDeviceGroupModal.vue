							...
							<div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								<div class="sm:flex sm:items-start">
									...
									>
										...
										>
											Edit Device Group {{ props.deviceGroup.name }}
										</DialogTitle>
										<div class="mt-2 space-y-6">
											<p class="text-sm text-gray-500">
												Here, you can update the name of the Device Group.
											</p>
...
<script lang="ts" setup>
	...
	interface Emits {
		(e: 'close'): void
		(e: 'edited', deviceGroup: DeviceGroup): void
	}
	...
	async function edit() {
		...
		try {
			const deviceGroup = await $api<DeviceGroup>(
				`/device-groups/${props.deviceGroup.id}`,
				{
					method: 'PATCH',
					body: data,
				}
			)
			emit('edited', deviceGroup)
			close()
		} catch (err) {
