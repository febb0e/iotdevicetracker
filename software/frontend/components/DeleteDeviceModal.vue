					...
					>
						<div class="sm:flex sm:items-start">
							...
							<div class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left">
								...
								>
									Delete Device {{ props.device.name }}
								</DialogTitle>
								<div class="mt-2">
									<p class="text-sm text-gray-500">
										Are you sure you permanently want to delete Device
										{{ props.device.name }}? All the data will be permanently
...
<script lang="ts" setup>
	...
	async function deleteDevice() {
		...
		try {
			await $api(`/devices/${props.device.id}`, {
				method: 'DELETE',
			})
			emit('deleted')
			close()
