					...
					>
						<div class="sm:flex sm:items-start">
							...
							<div class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left">
								...
								>
									Delete Device Group {{ props.deviceGroup.name }}
								</DialogTitle>
								<div class="mt-2">
									<p class="text-sm text-gray-500">
										Are you sure you permanently want to delete Device Group
										{{ props.deviceGroup.name }}? All the data will be
...
<script lang="ts" setup>
	...
	async function deleteGroup() {
		...
		try {
			await $api(`/device-groups/${props.deviceGroup.id}`, {
				method: 'DELETE',
			})
			emit('deleted')
			close()
