									...
									>
										...
										<div class="mt-2 space-y-6">
											<p class="text-sm text-gray-500">
												Device Groups contain multiple IoT Devices. You can
												later add other users to the device group, to give them
												permission to access devices in this group.
											</p>
...
<script lang="ts" setup>
	...
	interface Emits {
		(e: 'close'): void
		(e: 'created', deviceGroup: DeviceGroup): void
	}
	...
	async function create() {
		...
		try {
			const deviceGroup = await $api<DeviceGroup>('/device-groups', {
				method: 'POST',
				body: data,
			})
			emit('created', deviceGroup)
			close()
		} catch (err) {
